...
namespace DTPortal.Core.Services
{
    ...
    CreateJourney_FromConfig(UserLookupItem userInfo)
        ...
        private JourneyRequest BuildJourneyRequest(UserLookupItem userInfo)
        {
            ...
            var consent = !string.IsNullOrEmpty(consentConfig) ? consentConfig : "false";
            ...
            GetAccessTokenRequest request, string authHeader, string type)
        {
            ...
            if (request.grant_type == OAuth2Constants.AuthorizationCode)
            {
                ...
                if (accesstoken.Scopes.Contains(OAuth2Constants.openid))
                {
                    ...
                    if (attributes.Contains("birthdate"))
                        subscriberInfo.Birthdate = subscriberProfile.Birthdate;

                    if (attributes.Contains("name"))
                        subscriberInfo.Name = subscriberProfile.Name;

                    if (attributes.Contains("phone"))
                        subscriberInfo.Phone = subscriberProfile.Phone;

                    if (attributes.Contains("email"))
                        subscriberInfo.Email = subscriberProfile.Email;

                    if (attributes.Contains("gender"))
                        subscriberInfo.Gender = subscriberProfile.Gender;

                    if (attributes.Contains("id_document_type"))
                        subscriberInfo.IdDocumentType = subscriberProfile.IdDocumentType;

                    if (attributes.Contains("id_document_number"))
                        subscriberInfo.IdDocumentNumber = subscriberProfile.IdDocumentNumber;

                    if (attributes.Contains("loa"))
                        subscriberInfo.Loa = subscriberProfile.Loa;

                    if (attributes.Contains("country"))
                        subscriberInfo.Country = subscriberProfile.Country;
                    ...
                    if (attributes.Contains("currentNationality"))
                        subscriberInfo.CurrentNationality = subscriberProfile.CurrentNationality;

                    if (attributes.Contains("genderEn"))
                        subscriberInfo.GenderEn = subscriberProfile.GenderEn;

                    if (attributes.Contains("occupationEn"))
                        subscriberInfo.OccupationEn = subscriberProfile.OccupationEn;

                    if (attributes.Contains("emiratesIdNumber"))
                        subscriberInfo.EmiratesIdNumber = subscriberProfile.EmiratesIdNumber;

                    if (attributes.Contains("issueDate"))
                        subscriberInfo.IssueDate = subscriberProfile.IssueDate;

                    if (attributes.Contains("expiryDate"))
                        subscriberInfo.ExpiryDate = subscriberProfile.ExpiryDate;

                    if (attributes.Contains("passportType"))
                        subscriberInfo.PassportType = subscriberProfile.PassportType;

                    if (attributes.Contains("passportNo"))
                        subscriberInfo.PassportNo = subscriberProfile.PassportNo;

                    if (attributes.Contains("documentNationalityAbbr"))
                        subscriberInfo.DocumentNationalityAbbr = subscriberProfile.DocumentNationalityAbbr;

                    if (attributes.Contains("documentNationality"))
                        subscriberInfo.DocumentNationality = subscriberProfile.DocumentNationality;

                    if (attributes.Contains("passportIssueDate"))
                        subscriberInfo.PassportIssueDate = subscriberProfile.PassportIssueDate;

                    if (attributes.Contains("passportExpiryDate"))
                        subscriberInfo.PassportExpiryDate = subscriberProfile.PassportExpiryDate;
            ...
            (string authnToken)
        {
            ...
            */
            ...
            try
            {
                _logger.LogInformation($"Calling User Preference API for SUID: {tempSession.UserId}");
                var prefClient = _httpClientFactory.CreateClient();
                string url = configuration["APIServiceLocations:UserPreferenceBaseAddress"]
                             + "api/get/subscriber/preferences/by/suid/" + tempSession.UserId;
            ...
            {
                _logger.LogError($"Unable to retrive user preferences for suid {tempSession.UserId}");
                _logger.LogError(ex.Message);
            }
