...
namespace DTPortal.IDP.Controllers
{
    ...
    public class AuthenticationController : BaseController
    {
            ...
            ([FromBody][Required] VerifyUserAuthDataRequest requestObj)
        {
            ...
            {
                _logger.LogError("Exception Occurred in VerifyUserAuthData: {0}", ex);
                var response = new APIResponse
                {
                    Success = false,
            ...
            [FromForm][Required] GetAccessTokenRequest request)
        {
            ...
            try
            {
                ...
                {
                    if(request.refresh_token.Trim().Length == 0)
                    {
                        return BadRequest(new DTOs.ErrorResponseDTO
                        {
                            error = "invalid_request",
                            error_description = "refresh_token cannot be empty"
                        });
                    }
                }
                ...
                if (idpConfiguration == null)
                {
                    ...
                    return Ok(
                        ...
                        {
                            error = "server_error",
                            error_description = "IDP configuration missing"
                        });
                }
                ...
                if (openIdCommonConfig == null)
                {
                    return Ok(
                        ...
                        {
                            error = "server_error",
                            error_description = "Invalid IDP configuration"
                        });
                }
                        ...
                        StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        try
                        {
                            ...
                            if (!decoded.Contains(":"))
                            {
                                ...
                                {
                                    error = "invalid_client",
                                    error_description = "Invalid client credential format"
                                });
                            }
                        }
                        catch
